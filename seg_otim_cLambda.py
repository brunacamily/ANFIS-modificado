...
def validacao_cruzada(dataset_cfg, abordagem, lambda_param=0):
    """Executa validação cruzada k-fold para o dataset."""
    accuracies = []
    dataset_name = dataset_cfg["name"]
    n_splits = dataset_cfg["n_splits"]
    print(f"\n=== Dataset: {dataset_name} | Abordagem: {abordagem} | Lambda: {lambda_param} ===")
    ...
    print(f"\n\nDataset: {dataset_name} | Abordagem: {abordagem} | Lambda: {lambda_param} | Média final: {media:.4f}")
    return media
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print("Uso: python seg_otim.py <abordagem> <dataset_id> <lambda>")
        sys.exit(1)

    abordagem_input = sys.argv[1]
    dataset_id = sys.argv[2]
    try:
        LAMBDA_VALOR = float(sys.argv[3])
    except ValueError:
        print("O valor de lambda deve ser um número (int ou float).")
        sys.exit(1)
    ...
    resultados = []
    media_acc = validacao_cruzada(dataset_cfg, abordagem_input, LAMBDA_VALOR)
    resultados.append({
    ...
    output_file = OUTPUT_DIR / f"{dataset_cfg['name']}_{LAMBDA_VALOR}.xlsx"
